...
// GOLINKS_HOME lets personal data live outside the code folder (default: the code folder).
const HOME_DIR = process.env.GOLINKS_HOME ? path.resolve(process.env.GOLINKS_HOME) : ROOT;
...
// Older files had a flat `collection` string per link; it became the `folder` path.
// Returns true when something changed so the caller can write the file back.
function migrateLinks(links) {
  let changed = false;
  for (const l of links) {
    if (!l || typeof l !== 'object') continue;
    if ('collection' in l) {
      if (!l.folder && l.collection) l.folder = normalizeFolder(l.collection);
      delete l.collection;
      changed = true;
    }
    if (!('folder' in l)) { l.folder = null; changed = true; }
  }
  return changed;
}
...
class Store extends EventEmitter {
  ...
  load(name) {
    ...
    if (name === 'links' && migrateLinks(value)) this.write('links', value);
    return value;
  }
  ...
  // Every stored link, including the ones in the trash. Most callers want `live`.
  get links() { return this.data.links; }
  // Links not in the trash: what search, views, counts and exports work on.
  get live() { return this.data.links.filter((l) => !l.deleted); }
  // Soft-deleted links (link.deleted holds the time they were trashed), newest first.
  get trashed() { return this.data.links.filter((l) => l.deleted).sort((a, b) => Date.parse(b.deleted) - Date.parse(a.deleted)); }
  ...
  findByKeyword(keyword) {
    ...
    // A trashed link releases its keyword; restoring it drops the keyword if taken meanwhile.
    return this.links.find((l) => !l.deleted && l.keyword && l.keyword.toLowerCase() === k) || null;
  }
  ...
  findFolder(path) {
    ...
    return this.folders.find((f) => f && String(f.path).toLowerCase() === k) || null;
  }

  // Remember a folder (and its ancestors) so it exists even without links. Returns the
  ...
  ensureFolder(path) {
    ...
    for (let i = 1; i <= segs.length; i++) {
      ...
      if (!this.findFolder(a)) this.folders.push({ path: a, created: new Date().toISOString() });
    }
    return this.findFolder(p).path;
  }
}
